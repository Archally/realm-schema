...
export const ID_RE = /^([a-z][a-z0-9-]*\.)?[A-Z]{1,5}\d{3,}$/;
...
export const DATA_FILE_TO_SCHEMA = {
  ...
  "migrations.yaml":              "migration.schema.yaml",
...
export function loadSchemaRegistry(schemaDir) {
  const schemaFiles = walkFiles(schemaDir, f => /\.schema\.(yaml|yml)$/i.test(f));
  const registry = new Map();
  for (const filePath of schemaFiles) {
    const rel = toPosix(path.relative(schemaDir, filePath));
...
export function makeAjv(registry) {
  const ajv = new Ajv2020({ allErrors: true, strict: false, allowUnionTypes: true, validateSchema: false });
